...
main = function() {
    ...
    dbWriteTable (db_connection, "biologging.tmp_all_deployments", value=pg_all_deployments, append=TRUE, row.names=FALSE);
    ...
    for (wc_id in all_deployments$wc_id) {
      ...
      if (nrow(all_locations) > 0) {
        ...
        dbWriteTable (db_connection, "biologging.tmp_all_locations", value=pg_all_locations, append=TRUE, row.names=FALSE);
    ...
    dbWriteTable (db_connection, "biologging.tmp_all_deployment_profiles", value=pg_all_deployment_profiles, append=TRUE, row.names=FALSE);
    ...
    for (wc_id in all_deployment_profiles$wc_id) {
      ...
      if (nrow(all_profiles) > 0) {
        ...
        dbWriteTable (db_connection, "biologging.tmp_all_profiles", value=pg_all_profiles, append=TRUE, row.names=FALSE);
      }
    }
